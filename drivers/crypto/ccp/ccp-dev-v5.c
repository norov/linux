					...
					unsigned long *lsb_pub)
{
	 ...
	 * ordinal given to us in lsb_cnt, find possible LSB with the
	 * corresponding bit in the aggregation mask is set.
	 */
	for (i = 0; i < ccp->cmd_q_count; i++) {
		struct ccp_cmd_queue *cmd_q = &ccp->cmd_q[i];

		if (bitmap_weight(cmd_q->lsbmask, MAX_LSB_CNT) != lsb_cnt)
			continue;

		bitno = find_first_and_bit(cmd_q->lsbmask, lsb_pub, MAX_LSB_CNT);
		if (bitno >= MAX_LSB_CNT)
			return -EINVAL;

		/* We found an available LSB that this queue can access */
		cmd_q->lsb = bitno;
		__clear_bit(bitno, lsb_pub);
