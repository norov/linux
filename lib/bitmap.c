...
#define BITMAP_WEIGHT(EXPRESSION, bits)	\
({										\
	unsigned int __bits = (bits), k, w = 0;					\
										\
	for (k = 0; k < __bits / BITS_PER_LONG; k++)				\
		w += hweight_long(EXPRESSION);					\
										\
	if (__bits % BITS_PER_LONG)						\
		w += hweight_long(EXPRESSION & BITMAP_LAST_WORD_MASK(__bits));	\
										\
	w;									\
})
...
{
	return BITMAP_WEIGHT(bitmap[k], bits);
}
...
unsigned int __bitmap_weight_and(const unsigned long *bitmap1,
				const unsigned long *bitmap2, unsigned int bits)
{
	return BITMAP_WEIGHT(bitmap1[k] & bitmap2[k], bits);
}
