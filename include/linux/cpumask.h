...
/**
 * for_each_cpu_and_andnot - iterate over every cpu present in one mask, excluding
 *			 those present in another.
 * @cpu: the (optionally unsigned) integer iterator
 * @mask1: the first cpumask pointer
 * @mask2: the second cpumask pointer
 ...
 * After the loop, cpu is >= nr_cpu_ids.
 */
#define for_each_cpu_and_andnot(cpu, mask1, mask2, mask3)			\
	for_each_and_andnot_bit(cpu, cpumask_bits(mask1),			\
					cpumask_bits(mask2),			\
					cpumask_bits(mask3), nr_cpumask_bits)
