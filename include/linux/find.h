...
unsigned long _find_first_and_bit(const unsigned long *addr1, const unsigned long *addr2,
				 ...
				 unsigned long size)
{
	...
	return _find_first_and_bit(addr1, addr2, size, 0UL);
}
#endif

/**
 * find_first_andnot_bit - find the first set bit in both memory regions
 * @addr1: The first address to base the search on
 * @addr2: The second address to base the invert-search on
 * @size: The bitmap size in bits
 *
 * Returns the bit number for the next set bit
 * If no bits are set, returns @size.
