...
unsigned long _find_and_set_next_bit(volatile unsigned long *addr, unsigned long nbits,
				unsigned long start);
unsigned long _find_and_set_bit_lock(volatile unsigned long *addr, unsigned long size);
...
/**
 * find_and_set_bit - Find a zero bit and set it
 * @addr: The address to base the search on
 ...
 * the aquired bit belongs to the caller exclusively.
 *
...
/**
 * find_and_set_bit_wrap - find and set bit starting at @ofset, and wrapping around zero
 * @addr: The first address to base the search on
 * @size: The bitmap size in bits
 * @offset: The bitnumber to start searching at
 *
...
unsigned long find_and_set_bit_wrap(volatile unsigned long *addr,
					unsigned long size, unsigned long offset)
{
	unsigned long bit = find_and_set_next_bit(addr, size, offset);
...
/**
 * find_and_set_bit_lock - Find a zero bit and set it
 * @addr: The address to base the search on
 * @nbits: The bitmap nbits in bits
 *
 * This function is designed to operate in concurrent access environment.
...
/**
 * find_and_clear_bit - Find a set bit and clear it
 * @addr: The address to base the search on
 * @nbits: The bitmap nbits in bits
 *
 * This function is designed to operate in concurrent access environment.
...
/**
 * find_and_clear_next_bit - Find a set bit next after a given @offset, and clear it
 * @addr: The address to base the search on
 * @nbits: The bitmap nbits in bits
 ...
 *
 * Because of concurrency and volatile nature of underlying bitmap, it's not
 * guaranteed that the bit found is the 1st bit in the bitmap next to a given
 * one. It's also not guaranteed that if @nbits is returned, there's no set
 * bits after @offset.
 *
 * The function does guarantee that if returned value is in range [@offset .. @nbits),
 * the aquired bit belongs to the caller exclusively.
